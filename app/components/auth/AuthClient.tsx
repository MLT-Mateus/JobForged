...
export function LoginClient() {
  ...
  return <AuthShell compact>
    ...
    <form className="auth-form" noValidate onSubmit={submit}>
      <TextField label="E-mail ou CPF" requiredLabel="Obrigatório" icon={Mail} value={values.identifier} onChange={(event)=>updateIdentifier(event.target.value)} error={errors.identifier} autoComplete="username" inputMode="email" placeholder="nome@empresa.com.br ou 000.000.000-00"/>
      <TextField label="Senha" requiredLabel="Obrigatório" icon={LockKeyhole} type="password" value={values.password} onChange={(event)=>{setValues((current)=>({...current,password:event.target.value}));if(errors.password)setErrors((current)=>({...current,password:undefined}))}} error={errors.password} autoComplete="current-password" placeholder="Digite sua senha"/>
...
export function SignupClient() {
  ...
  return <AuthShell>
    ...
    <form className="auth-form auth-form--signup" noValidate onSubmit={submit}>
      <TextField label="Nome da empresa" requiredLabel="Obrigatório" icon={Building2} value={values.company} maxLength={120} onChange={(event)=>set("company",event.target.value)} error={errors.company} autoComplete="organization" placeholder="Razão social ou nome fantasia"/>
      <div className="auth-field-row"><TextField label="CNPJ" requiredLabel="Obrigatório" icon={Building2} value={values.cnpj} onChange={(event)=>set("cnpj",maskCnpj(event.target.value))} error={errors.cnpj} autoComplete="off" inputMode="numeric" placeholder="00.000.000/0000-00"/><TextField label="Telefone" requiredLabel="Obrigatório" icon={Phone} value={values.phone} onChange={(event)=>set("phone",maskPhone(event.target.value))} error={errors.phone} autoComplete="tel" inputMode="tel" placeholder="(00) 00000-0000"/></div>
      <TextField label="E-mail do administrador" requiredLabel="Obrigatório" icon={Mail} value={values.email} onChange={(event)=>set("email",event.target.value.slice(0,120))} error={errors.email} autoComplete="email" inputMode="email" placeholder="administrador@empresa.com.br" helpText={errors.email ? undefined : "Este e-mail será utilizado para acessar e administrar a conta da empresa."}/>
      <div className="auth-field-row"><TextField label="Senha" requiredLabel="Obrigatório" icon={LockKeyhole} type="password" value={values.password} onChange={(event)=>set("password",event.target.value)} error={errors.password} autoComplete="new-password" placeholder="Crie uma senha segura"/><TextField label="Confirmar senha" requiredLabel="Obrigatório" icon={LockKeyhole} type="password" value={values.confirmPassword} onChange={(event)=>set("confirmPassword",event.target.value)} error={errors.confirmPassword} autoComplete="new-password" placeholder="Digite a senha novamente"/></div>
